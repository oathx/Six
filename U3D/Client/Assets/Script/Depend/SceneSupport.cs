...
public class SceneSupport : MonoBehaviourSingleton<SceneSupport>
{
	...
	/// <param name="szName">Size name.</param>
	public virtual bool		LoadScene(int nSceneID, bool bSteam, string szUrl, string szName)
	{
		try{
			SceneID = nSceneID;
			...
			if (bSteam)
			{
				m_ResourceManager.LoadAssetBundleFromStream(szUrl, 
				                                            delegate(string szAssetURL, AssetBundleResource abResource) {
	#if OPEN_DEBUG_LOG
	...
	public virtual bool		LoadScene(int nSceneID, bool bStream, string szUrl, 
	                               string szName, List<string> aryPreLoad, SceneLoadingCallback callback)
	{
		SendEvent(nSceneID, CmdEvent.CMD_SCENE_LOADSTART, 0);

		// current pre load resource count
		int nCurrentLoadCount = 0;

		if (aryPreLoad.Count > 0)
		{
			...
			for (int i = 0; i < aryPreLoad.Count; i++)
			{
				m_ResourceManager.LoadAssetBundleFromStream(aryPreLoad[i],
															...
															// calc pre load preogress
															float fProgress = (float)nCurrentLoadCount / (float)aryPreLoad.Count;

															// proc callback 
															callback(
																fProgress, aryPreLoad[nCurrentLoadCount]
																);

																nCurrentLoadCount++;

																if (nCurrentLoadCount == aryPreLoad.Count)
																	return LoadScene(nSceneID, bStream, szUrl, szName);
	...
	public virtual bool		LoadNavMesh(string szAssetName)
	{
		TextAsset asset = m_ResourceManager.GetAsset<TextAsset>(WUrl.AssetNavmeshPath, szAssetName);
		if (asset)
		{
