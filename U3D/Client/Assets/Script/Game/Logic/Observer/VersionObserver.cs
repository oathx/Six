...
public class VersionObserver : IEventObserver
{
	...
	public void 		UpdateVersion(string text)
	{
		...
		if (v.Package != default(List<VersionStruct.VersionPackage>) && v.Package.Count > 0)
		{
			Queue<HttpWork> 
			...
			UISystem.GetSingleton().Box(BoxStyle.YES, message, 0, 
				...
				if (bFlag)
				{
					...
					File.WriteAllText(
						string.Format("{0}/Version.txt", Application.dataPath), JsonMapper.ToJson(v)
						);
#endif
					Download(workQueue);
				}
	...
	protected void OpenAndRegisterSqlFactory()
	{
		...
		GameSqlLite.GetSingleton ().RegisterSqlPackageFactory (
			typeof(SqlSystem).Name, new DefaultSqlPackageFactory<SqlSystem>()
			);
		GameSqlLite.GetSingleton ().RegisterSqlPackageFactory (
		...
		GameSqlLite.GetSingleton ().RegisterSqlPackageFactory (
			typeof(SqlMagic).Name, new DefaultSqlPackageFactory<SqlMagic>()
			);
		GameSqlLite.GetSingleton ().RegisterSqlPackageFactory (
			typeof(SqlSkill).Name, new DefaultSqlPackageFactory<SqlSkill>()
			);
	}
	...
	protected void RegisterUnitEntityFactory()
	{
		...
		if (monsterManager) 
		{
			monsterManager.RegisterEntityFactory (
				EntityType.ET_MONSTER.ToString (), new MonsterEntityFactory ()
				);
			monsterManager.RegisterEntityFactory (
				EntityType.ET_NPC.ToString (), new MonsterEntityFactory ()
				);
		}
	}
}
